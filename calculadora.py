...
class app:
    def __init__(self):
        ...
        Label(self.root,text="TU CONTRASEÑA:").place(x=10,y=30)
        Entry(self.root,textvariable=self.your_password,font=('arial 20'),width=58).place(x=10,y=50)
        Label(self.root,text="LONGITUD:").place(x=8,y=110)
        self.stateLabel = Label(self.root,text="",width=127)
        self.stateLabel.place(x=2,y=141)
        self.btnCreate = Button(self.root,text="CREAR CONTRASEÑA",width=123,height=2,bg="gray86",command=self.init_task)
        self.btnCreate.place(x=12,y=168)
        Button(self.root,text="GUARDAR",width=60,height=2,bg="gray86",command=self.save_password).place(x=12,y=218)
        Button(self.root,text="COPIAR",width=60,height=2,bg="gray86",command=self.copy).place(x=453,y=218)
        self.len=ttk.Combobox(self.root,width=5,state="readonly")
        self.len.place(x=80,y=110)
        Label(self.root,text="MINÚSCULAS:").place(x=168,y=110)
        self.min_low=ttk.Combobox(self.root,width=5,state="readonly")
        self.min_low.place(x=255,y=110)
        Label(self.root,text="MAYUSCULAS:").place(x=350,y=110)
        self.min_upp=ttk.Combobox(self.root,width=5,state="readonly")
        self.min_upp.place(x=440,y=110)
        Label(self.root,text="NUMEROS:").place(x=525,y=110)
        self.min_num=ttk.Combobox(self.root,width=5,state="readonly")
        self.min_num.place(x=595,y=110)
        Label(self.root,text="CARACTERES ESPECIALES:").place(x=680,y=110)
    ...
    def genera_password(self):
        ...
        self.stateLabel.configure(text="Generando contraseña...",fg="red")
        ...
        #FIN DE TAREA       
        self.stateLabel.configure(text="Contraseña Generada.",fg="blue")     
        if self.activated == False:
            self.btnCreate.configure(text="CREAR CONTRASEÑA",command=self.init_task)
        self.your_password.set(pswrd)
        self.activated = True
    ...
    def save_password(self):
        if len(self.your_password.get())>0 and self.running == False:
            question = messagebox.askquestion("ESTAS SEGURO?",'''Guardar la contraseña en texto sin formato no es una práctica recomendada.
De todos modos, ¿quieres continuar?''')
            if question == "yes":
    ...
    def init_task(self):
        ...
        if int(self.min_low.get()) + int(self.min_upp.get()) + int(self.min_num.get()) + int(self.min_char.get())<= int(self.len.get()):
            self.btnCreate.configure(text="CANCELAR PROCESO",command=self.cancel_process)
            t = threading.Thread(target=self.genera_password)
            t.start()
        else:
            messagebox.showwarning("ERROR","LONGITUD NO VÁLIDA.")
